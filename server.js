...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    // Validar se colocou nome completo (pelo menos duas palavras)
    const nomeParts = trimmedNome.split(/\s+/);
    if (nomeParts.length < 2) {
      return res.status(400).json({ error: 'Por favor, digite seu nome completo (nome e sobrenome).' });
    }
    ...
    const exists = await dbExecute({ sql: 'SELECT id FROM usuarios WHERE email = ?', args: [trimmedEmail] });
    if (exists.rows.length > 0) {
      return res.status(400).json({ error: 'Este e-mail já está cadastrado.' });
    }

    // Hash da senha e inserção como ALUNO
    const hash = await hashSenha(senha);
    const result = await dbExecute({
      sql: 'INSERT INTO usuarios (nome, email, senha_hash, role) VALUES (?, ?, ?, ?)',
      args: [trimmedNome, trimmedEmail, hash, 'aluno'],
    });
    ...
    res.status(201).json({
      token,
      user: { id: userId, nome: trimmedNome, email: trimmedEmail, role: 'aluno' },
    });
  } catch (err) {
...
app.delete('/api/admin/alunos/:id', middlewareAuth, middlewareRole('admin'), async (req, res) => {
  try {
    const { id } = req.params;

    // Verificar se o usuário existe
    const exists = await dbExecute({ sql: 'SELECT id, role FROM usuarios WHERE id = ?', args: [id] });
    if (exists.rows.length === 0) {
    ...
    // Executa exclusão das tabelas vinculadas em lote
    await dbBatch([
      { sql: 'DELETE FROM respostas WHERE aluno_id = ?', args: [id] },
      { sql: 'DELETE FROM projetos_codigo WHERE aluno_id = ?', args: [id] },
      { sql: 'DELETE FROM usuarios WHERE id = ?', args: [id] }
    ]);

    res.json({ message: 'Perfil e todos os dados vinculados foram removidos com sucesso.' });
  } catch (err) {
