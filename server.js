...
app.delete('/api/admin/alunos/:id', middlewareAuth, middlewareRole('admin'), async (req, res) => {
  try {
    ...
    await dbBatch([
      { sql: 'DELETE FROM respostas WHERE aluno_id = ?', args: [numericId] },
      { sql: 'DELETE FROM projetos_codigo WHERE aluno_id = ?', args: [numericId] },
      { sql: 'DELETE FROM usuarios WHERE id = ?', args: [numericId] }
    ]);
